...
def main(config_file):
    ...
    run_dir = os.getcwd()  # or set to desired output folder
    log_file = os.path.join(run_dir, cfg['paths']['log_dir'], 'eval.log')
    logging.basicConfig(
        level=cfg['output_configuration'].get('logging_level', 'INFO'),
    ...
    # load data csv and select split
    df = pd.read_csv(cfg['paths']['data_csv'])
    split_name = cfg['evaluation']['split']
    eval_df = df[df['split'] == split_name]

    # dataset and loader
    transforms = get_transforms(is_train=False)
    dataset = MultiGraderDataset(
        video_paths=eval_df[cfg['training']['datamodule']['video_col']].tolist(),
        labels=eval_df[cfg['training']['datamodule']['label_cols']].values.tolist(),
        transform=transforms,
        hierarchical=(cfg['training']['num_subclasses'] > 0)
    )
    sampler = DistributedSampler(dataset) if GPUSetup.is_distributed() else None
    loader = DataLoader(
        dataset,
    ...
    engine = EvalEngine(
        ...
        cfg=cfg,
        device=device,
        save_attention=cfg['evaluation'].get('save_attention', False),
        output_dir=run_dir
    )
